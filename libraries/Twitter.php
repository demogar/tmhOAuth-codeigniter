...
class Twitter {
	...
	private $consumer_key; // twitter
	private $consumer_key_secret; // twitter
	private $configs; // array of consumer key and secret
	public $tmhOAuth;
	public $userdata; // user data
	public $tempImage; // temporary avatar image, loaded locally
	
	function __construct()
	{
		$this->CI =& get_instance();
		
		// Loading the consumer key and consumer key secret from the config file
		$this->consumer_key = $this->CI->config->item("consumer_key");
		$this->consumer_key_secret = $this->CI->config->item("consumer_key_secret");
		
		$this->configs = array(
			'consumer_key'		=> $this->consumer_key,
			'consumer_secret'	=> $this->consumer_key_secret
		);
		
		// Init the tmhOAuth
		$this->tmhOAuth = new tmhOAuth($this->configs);
	}
	
	/**
	 * Get the Access Token and stores it in the session for future usage
	 *
	...
	{
		$this->tmhOAuth->config['user_token']  = $this->CI->session->userdata("o_oauth_token");
		$this->tmhOAuth->config['user_secret'] = $this->CI->session->userdata("o_oauth_token_secret");
		
		$this->tmhOAuth->request('POST', $this->tmhOAuth->url('oauth/access_token', ''), array(
			'oauth_verifier' => $oauth_verifier
		));
		
		$response_access = $this->tmhOAuth->extract_params($this->tmhOAuth->response['response']);
		$access_tokens = array(
			"a_oauth_token"			=> $response_access["oauth_token"],
			"a_oauth_token_secret"	=> $response_access["oauth_token_secret"]
		);
		
		$this->CI->session->set_userdata($access_tokens);
	}
	
	/**
	 * Get the OAuth Token. It will be used to obtain the Access Token
	 *
	...
	public function getOauthToken($callback)
	{
		...
		if ( $this->tmhOAuth->response['code'] == 200 ) {
			...
			$oauth_tokens	= array(
				"o_oauth_token"			=> $response["oauth_token"],
				"o_oauth_token_secret"	=> $response["oauth_token_secret"]
			);
			$this->CI->session->set_userdata($oauth_tokens);
			header("Location: " . $this->tmhOAuth->url("oauth/authorize", "") . "?oauth_token=" . $response["oauth_token"]);
		} else {
			show_error( $this->tmhOAuth->pr(htmlentities($this->tmhOAuth->response['response'])) );
		}
	}
	
	/**
	 * Stores the User's data under the userdata public var
	 *
	 * @return void
	 * @author demogar
	 */
	public function getUserData()
	{
		...
		if ( $this->tmhOAuth->response["code"] != 200 ) {
			show_error( $this->tmhOAuth->response["code"] );
		}
	}
	 ...
	 * Get the Avatar URL
	 *
	 * @param string $size = size of the image (normal or bigger)
	 * @return void
	 * @author demogar
	 */
	public function getAvatarUrl($size = "bigger")
	{
		$possible_sizes = array("bigger", "normal", "mini");
		if ( $this->userdata->screen_name != NULL && in_array($size, $possible_sizes) ) {
			...
			if ( $this->tmhOAuth->response["code"] == 302 ) {
				// This contains the URL for the image
				return $this->tmhOAuth->response["headers"]["location"];
			} else {
				show_error( $this->tmhOAuth->response["code"] );
			}
		} else {
			show_error("<p>You are not logged in or you specified a wrong size</p>");
		}
	}
	
	/**
	 * Get the Original Sized Avatar
	 *
	 * @return The original sized avatar
	 * @author demogar
	 */
	...
	/**
	 * Download the Avatar from Twitter and store it locally
	 *
	 * @return void
	 * @author demogar
	 */
	...
	{
		// Random image name
		$this->tempImage	= $this->userdata->screen_name . "_avatar.png";
		$this->path			= FCPATH . 'tmp/' . $this->tempImage;
		
		// Download image and save at the application/tmp folder
	...
	/**
	 * Add a user to the following list (add as a friend)
	 *
	 * @param string $user = the screen_name o id
	 * @param string $type = if we are adding it as a scree_name or as a id
	 * @return void
	 * @author demogar
	 */
	public function followUser($user = NULL, $type = 'screen_name')
	{
		if ( $user != NULL ) {
			// Already friends?
			if ( ! $this->checkFriendship($this->userdata->screen_name, $user) ) {
				$this->tmhOAuth->request('POST', $this->tmhOAuth->url('1/friendships/create'), array(
					$type	=> $user
				), true, true);
				
				if ( $this->tmhOAuth->response['code'] != 200 ) {
					$error = "<p>Unable to add {$user} to your list of friends.</p>";
					$error .= "<p><strong>Error code:</strong> " . $this->tmhOAuth->response['code'] . "</p>";
					$error .= "<p><strong>Error:</strong> " . $this->errorMessage() . "</p>";
					show_error($error);
				}
			}
		} else {
			show_error("You must specify a user to follow");
		}
	}
	
	/**
	 * Verify the friendship between two different users
	 *
	 * @param string $user1 = screen name for the source user
	 * @param string $user2 = screen name for the target user
	 * @return boolean = return if the source user is following the target user
	 * @author demogar
	 */
	public function checkFriendship($user1, $user2)
	{
		$this->tmhOAuth->request("GET", $this->tmhOAuth->url("1/friendships/show"), array(  
			'source_screen_name'	=> $user1,
			'target_screen_name'	=> $user2
		));
		
		if ( $this->tmhOAuth->response['code'] == 200 ) {
			$data = json_decode($this->tmhOAuth->response["response"]);
			
			if ( $data->relationship->source->following == 1 || $data->relationship->source->following === 't' || $data->relationship->source->following === 'true' ) {
				return TRUE;
			} else {
				return FALSE;
			}
		} else {
			$error = "<p>Errow while verifying the friendship between {$user1} and {$user2}.</p>";
			$error .= "<p>Error code: " . $this->tmhOAuth->response['code'] . "</p>";
			$error .= "<p>Error message: " . $this->errorMessage() . "</p>";
			show_error( $error );
		}
	}
	
	/**
	 * Commit the avatar to Twitter
	 *
	 * @param string $path = local path for the image
	 * @param string $image = image filename
	 * @return void
	 * @author demogar
	 */
	public function sendAvatarToTwitter($path = NULL)
	{
		if ( file_exists($path) && $path != NULL) {
			$this->tmhOAuth->request('POST', $this->tmhOAuth->url('1/account/update_profile_image'), array(
				'image' => '@' . $path . ';type=image/png'
			), true, true);
			
			// Error
			if ( $this->tmhOAuth->response['code'] != 200 ) {
				$error = "<p>Error while commiting the new Profile Picture.</p>";
				$error .= "<p><strong>Error code:</strong> " . $this->tmhOAuth->response['code'] . "</p>";
				$error .= "<p><strong>Error:</strong> " . $this->errorMessage() . "</p>";
				$error .= "<p><strong>Path:</strong> " . $path;
				show_error($error);
			}
		} else {
			show_error("Your avatar doesn't exist in our system");
		}
	}
	
	/**
	 * Will send a tweet with the Text specified in the first parameter
	 *
	 * @param string $text = the tweet, max 140 chars
	 * @return void
	 * @author demogar
	 */
	...
	{
		$information = json_decode($this->tmhOAuth->response["response"]);
		return $information->error;
	}
	
	/**
	 * Will verify is the user is logged in already
	 *
	 * @return boolean
	 * @author demogar
	 */
	...
	{
		if ($this->CI->session->userdata('a_oauth_token') != NULL && $this->CI->session->userdata('a_oauth_token_secret') != NULL) {
			return TRUE;
		}
		return FALSE;
	}
...
/* Location: ./system/application/libraries/Twitter.php */
